...
function createAvatarPicker() {
  ...
  // Among Us 색상 순서에 맞춤: red, blue, green, pink, orange, yellow, black, white
  const colors = ['🔴', '🔵', '🟢', '🩷', '🟠', '🟡', '⚫', '⚪'];
...
async function loadProblem() {
  try {
    // 세션 코드와 함께 문제 요청 (세션의 3개 문제 중 랜덤)
    const response = await fetch(`${API_BASE}/problems/next?code=${sessionCode}`);
...
async function sendMessage() {
  ...
  try {
    ...
    if (answerToken && answerToken.startsWith('sample-token-')) {
      ...
      console.log('📡 API URL:', `${API_BASE}/messages`);
    }
